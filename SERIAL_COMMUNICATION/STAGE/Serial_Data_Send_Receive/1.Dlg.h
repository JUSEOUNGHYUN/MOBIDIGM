...
// Communication box data
	...
	// creating a message handler that will occur when using a user message. ** (important) **
...
public:
	...
	CComboBox m_combo_Comport_Select;	// ComboBox variable
	afx_msg void OnBnClickedSerialOpen();
	CListBox m_listSerialRespondData;	// CListBox member variable
	afx_msg void OnBnClickedSerialReceiveData();
	int CommSendData(BYTE* pData, int nLen);
};
