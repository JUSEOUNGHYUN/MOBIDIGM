...
void CMFCSerialRecevieDlg::OnBnClickedFileSelect()
{
	...
	*/
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		m_fp = fopen(m_strPathNamePure.c_str(), "rb");
	
		fseek(m_fp, 0, SEEK_END);
		m_len = ftell(m_fp);	// File Total size(Length)
...
LRESULT CMFCSerialRecevieDlg::OnMyMessage(WPARAM wParam, LPARAM lParam) {
	...
	if (message.size() != 0) {
		...
		// ---- Receiver ----
			...
			// 현재 프로그램의 실행위치에 mp3 저장
			// std::string first = strPathName.substr(0, pos);
			std::string NewPathName = "D:\\디렉터리\\MFC_Serial_Recevie\\x64\\Debug\\";
			std::string second = strFIlePathSubstrPure(strTemp);
			std::string m_WriteStrPathName = NewPathName + second;
		...
		// fopen -> move last positoin of file -> fwrite -> fclose
			...
			if (buffer_len < (SEND_DATA_LEN-1))
				TRACE("[buffer_len] %d\n", buffer_len);
			// 2. fwrite 
			fwrite(WriteByte, buffer_len, 1, m_wfp);
...
void CMFCSerialRecevieDlg::Send_Data()
{
	...
	int32_t read_len = 0;
	if (quotient > m_cnt) {
		read_len = fread(&str_c1[1], 1, SEND_DATA_LEN -1, m_fp);
		m_cnt++;
		...
		read_len = fread(&str_c1[1],  1, remainder,m_fp);
